...
public class GolfBall3
{
	...
	public static void main (String[] args)
	{
		...
		*/
		...
		for (int i=2; y[i-1] > 0.0; i++)
		{
		    ...
		    x[i] = ((2*x[i-1]) - (x[i-2]) + (accx(v[i-1],theta[i-1]) * (dt*dt))); //Verlet's is now used to determine position of ball]
		...
		while(theta[0] < (Math.PI/2.0))
		{
		    ...
		    */
		    ...
		    for (int i=2; y[i-1] > 0.0; i++)
		    {
			    ...
			    y[i] = (y[i-1] + (vy * dt)); // Verlet's won't work here
	...
	*/
		...
		for (int i=2; y45[i-1] > 0.0; i++)
		{
		    ...
		    x45[i] = ((2*x45[i-1]) - (x45[i-2]) + (accx(v[i-1],theta[i-1]) * (dt*dt)));
			y45[i] = ((2*y45[i-1]) - (y45[i-2]) + (accy(v[i-1],theta[i-1]) * (dt*dt)));
			xMax45 = x45[i];
	    }
